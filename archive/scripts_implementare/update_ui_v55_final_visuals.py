...
for line in lines:
    if "with tabs[2]:" in line:
        ...
        # Indentare dinamică bazată pe linia curentă
        base_indent = " " * (len(line) - len(line.lstrip()) + 4)
        i2 = base_indent + "    "
        i3 = i2 + "    "
        i4 = i3 + "    "
        
        # Începem blocul de cod
        ...
        # Calcul Progres (0.0 - 1.0 pentru ProgressColumn)
        ...
        # AFIȘARE TABEL CU FORMATĂRI VIZUALE
        new_lines.append(f"{i4}        st.dataframe(\n")
        new_lines.append(f"{i4}            c_v[['Articol', 'Planificat', 'Total Intrat', 'În Depozit', 'În Custodie', 'Realizat (PV)', 'Progres %']],\n")
        ...
        new_lines.append(f"{i4}                    'Status Execuție', help='Progres validat prin PV',\n")
        new_lines.append(f"{i4}                    format='%.1f%%', min_value=0, max_value=1\n")
