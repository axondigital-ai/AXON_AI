...
for line in lines:
    if "with tabs[2]:" in line:
        ...
        # Calculăm automat indentarea existentă (câte spații are linia 'with')
        base_indent = len(line) - len(line.lstrip())
        i1 = " " * (base_indent + 4) # Nivelul 1 (sub with)
        i2 = i1 + "    "             # Nivelul 2
        i3 = i2 + "    "             # Nivelul 3
        i4 = i3 + "    "             # Nivelul 4
        
        # Inserăm logica salvată cu indentarea perfectă
        new_lines.append(f"{i1}# --- AXON GESTIUNE V49 (STABIL) ---\n")
        ...
        new_lines.append(f"{i4}        c_df['% Progres'] = (c_df['Cantitate_Validata'] / c_df['Cantitate_Planificata']).fillna(0)\n")
        ...
        new_lines.append(f"{i4}        st.dataframe(c_view[['Articol', 'Planificat', 'Total Intrat', 'În Depozit', 'În Custodie', 'Realizat (PV)', '% Progres']], use_container_width=True, hide_index=True, column_config={{'% Progres': st.column_config.ProgressColumn('Progres', format='%.0f%%', min_value=0, max_value=1)}})\n")
