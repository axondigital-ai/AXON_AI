...
for line in lines:
    if "with tabs[2]:" in line:
        ...
        new_lines.append(f"{i3}st.subheader('📊 CONTROL OPERAȚIONAL EPC (STATUS PROIECT)')\n")
        
        # Agregare Live conform Regulii Generale
        ...
        new_lines.append(f"{i4}        c_df['Progres Real'] = (c_df['Cantitate_Validata'] / c_df['Cantitate_Planificata'] * 100).fillna(0)\n")
        ...
        # Formatăm coloana de progres ca text cu % pentru a fi citibilă instant
        new_lines.append(f"{i4}        c_v['Progres Real'] = c_v['Progres Real'].apply(lambda x: f'{{x:.1f}}%')\n")
        
        new_lines.append(f"{i4}        st.dataframe(c_v[['Articol', 'Planificat', 'Total Intrat', 'În Depozit', 'În Custodie', 'Realizat (PV)', 'Progres Real']], use_container_width=True, hide_index=True)\n")
        ...
        new_lines.append(f"{base_indent}except Exception as e: st.error(f'Eroare Gestiune: {{e}}')\n")
        skip = True
        continue
