...
for line in lines:
    if "with tabs[2]:" in line:
        ...
        # Injectăm stilul VERDE pentru barele de progres
        new_lines.append(f"{base_indent}st.markdown('<style>div[data-testid=\"stProgress\"] > div > div > div > div {{ background-color: #28a745 !important; }}</style>', unsafe_allow_html=True)\n")
        ...
        # Calculăm Progresul ca valoare numerică pentru bară
        ...
        new_lines.append(f"{i4}                     column_config={{'Progres': st.column_config.ProgressColumn('Status Execuție', format='%.1f%%', min_value=0, max_value=1)}})\n")
